...
class BlinkStats:
    ...
    def _summary(self, xs: list[float]) -> str:
        if not xs:
            return "n=0"
        return (
            f"n={len(xs)} mean={statistics.mean(xs):.6f}s "
            f"stdev={statistics.pstdev(xs):.6f}s "
            f"min={min(xs):.6f}s max={max(xs):.6f}s"
        )
...
def _find_time_and_channel_columns(fieldnames: list[str], channel: int) -> tuple[str, str]:
    # Saleae raw export typically has a time column like "Time [s]" or similar.
    ...
    # Digital columns are often "Channel 0", "Channel 1", etc.
...
def iter_samples_from_digital_csv(digital_csv_path: str, channel: int) -> Iterable[tuple[float, int]]:
    with open(digital_csv_path, "r", newline="") as f:
        ...
        for row in reader:
            ...
            v = _parse_bool_cell(row[chan_col])
            yield (t, v)
...
def main(argv: list[str]) -> int:
    p = argparse.ArgumentParser(
        description="Capture Saleae channel 0 and measure Arduino pin 13 blink timing."
    )
    p.add_argument(
        "--launch",
        action="store_true",
        help="Launch Logic 2 automatically instead of connecting to an already-running instance.",
    )
    p.add_argument(
        "--logic2-path",
        default=None,
        help="Optional path to Logic 2 binary/app when using --launch.",
    )
    ...
    p.add_argument("--duration", type=float, default=6.0, help="Capture duration seconds")
    p.add_argument("--sample-rate", type=int, default=1_000_000, help="Digital sample rate Sa/s")
    p.add_argument(
        "--threshold-volts",
        type=float,
        default=None,
        help="Digital threshold volts (Logic Pro only). Example: 3.3",
    )
    p.add_argument("--expected-high", type=float, default=1.0, help="Expected high time (s)")
    p.add_argument("--expected-low", type=float, default=1.0, help="Expected low time (s)")
    ...
    try:
        if args.launch:
            with automation.Manager.launch(
                application_path=args.logic2_path, port=args.port
            ) as manager:
...
def _run_capture_and_report(manager: "automation.Manager", args: argparse.Namespace) -> int:
    ...
    if device_id is None:
        print(
            "ERROR: no Saleae devices found. Connect a device or pass --device-id.",
            file=sys.stderr,
        )
        return 3
    ...
    with manager.start_capture(
        ...
        with tempfile.TemporaryDirectory(prefix="saleae_blink_") as outdir:
            ...
            stats = compute_blink_stats(iter_samples_from_digital_csv(digital_csv, channel=args.channel))

    print(stats.format_report())

    # Simple pass/fail checks vs expectation.
    ...
    print("PASS: blink timing within tolerance")
    return 0
