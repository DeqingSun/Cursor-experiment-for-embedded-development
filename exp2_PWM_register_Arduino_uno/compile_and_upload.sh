...
# Reuse the Arduino CLI + config already set up in exp1 to avoid duplication.
exp1_dir="${script_dir}/../exp1_LED_blink_on_Arduino_uno"
cli_root="${exp1_dir}/arduino_cli"
cli_bin="${cli_root}/bin/arduino-cli"
config_file="${cli_root}/arduino-cli.yaml"

if [[ ! -x "$cli_bin" ]]; then
  echo "ERROR: Arduino CLI not found at: $cli_bin" >&2
  echo "Run: ${exp1_dir}/install_arduino_cli.sh" >&2
  exit 1
fi

if [[ ! -f "$config_file" ]]; then
  echo "ERROR: Arduino CLI config not found at: $config_file" >&2
  echo "Run: ${exp1_dir}/setup_arduino_uno.sh" >&2
  exit 1
fi
...
sketch_dir="${script_dir}/pwm_pb3_2khz_80"
compile_only="false"
port="${PORT:-}"

if [[ ! -f "${sketch_dir}/pwm_pb3_2khz_80.ino" ]]; then
  echo "ERROR: Sketch not found at: ${sketch_dir}/pwm_pb3_2khz_80.ino" >&2
  exit 1
fi
...
build_dir="${cli_root}/.tmp/build_pwm_pb3_2khz_80"
rm -rf "$build_dir"
mkdir -p "$build_dir"
